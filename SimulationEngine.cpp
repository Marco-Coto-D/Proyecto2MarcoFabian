...
SimulationEngine::SimulationEngine(World& world, Logger& logger, int bandageHealAmount, int bandagePrice) : world(world), logger(logger) {
...
void SimulationEngine::run() {
    ...
    if (victory) {
        logger.logVictory();
        cout << "\n*** VICTORIA ***" << endl;
        cout << "Las tinieblas han sido derrotadas. " << world.getKnight().getName() << "es el heroe del reino." <<endl;
    } else {
        logger.logDefeat();
        cout << "\n*** DERROTA ***" << endl;
        cout << "La oscuridad cubre el reino. El nombre de " << world.getKnight().getName() << " se pierde en el olvido." <<endl;
    }
    ...
    while (localTurns < maxTurns && !gameOver) {
        ...
        if (encountered == nullptr) {
            ...
            if (v != nullptr) {
                ...
                logger.logInteraction("Mago amistoso " + m->getName() + " curo al caballero");
                k.heal(k.getMaxHealth());
                if (m->getIsUnique()) room->removeEntity(m);
            } else if (e != nullptr) {
                runCombat(e, room);
                if (gameOver) return;
            }
...
void SimulationEngine::runCombat(Enemy* enemy, Room* room) {
    ...
    bool isGoblin = (dynamic_cast<Goblin*>(enemy) != nullptr);
    bool isDragon = (dynamic_cast<Dragon*>(enemy) != nullptr);
    bool isMage = (dynamic_cast<Mage*>(enemy)   != nullptr);
    ...
    if (enemy->isDead()) {
        ...
        if (isGoblin) {
            goblinsKilled++;
            knight.addGold(10);
            cout << "El goblin caido suelta una pequena bolsa con 10 de oro." << endl;
            logger.logInteraction("Recogió 10 de oro del goblin caído");
        } else if (isMage) {
            magesKilled++;
        } else if (isDragon) {
            dragonsKilled++;
...
bool SimulationEngine::handleMainMenu(Room* room, Villager* villager, bool isDungeon, bool canExplore) {
    ...
    while (true) {
        ...
        if (canUseBandage) cout << " " << optBandage  << ". Usar vendaje (" << knight.getBandages() << " disponibles)" << endl;
...
void SimulationEngine::handleVillagerInteraction(Villager* villager) {
    ...
    int resp;
    if (!(cin >> resp)) { cin.clear(); cin.ignore(1000, '\n'); resp = 0; }
    ...
    if (resp == 1) {
        ...
        cout << "Opcion: ";
        int shopChoice;
        if (!(cin >> shopChoice)) { cin.clear(); cin.ignore(1000, '\n'); shopChoice = 0; }
        else cin.ignore(1000, '\n');
...
void SimulationEngine::handleInventory() {
    Knight& knight = world.getKnight();
    cout << "\n=== Inventario de " << knight.getName() << " ===" << endl;
    cout << "Vida: " << knight.getHealth() << " / " << knight.getMaxHealth() << endl;
    cout << "Oro: " << knight.getGold() << endl;
    cout << "Vendajes: " << knight.getBandages() << endl;
    cout << "Equipado: " << knight.getEquippedWeapon().getName() << " (base: " << knight.getEquippedWeapon().getBaseDamage() << ", tipo: " << knight.getEquippedWeapon().getStrategyType() << ")" << endl;
    cout << "Armadura: " << knight.getEquippedArmor().getName() << " (reduccion: " << knight.getEquippedArmor().getDamageReduction() << "%)" << endl;
    cout << "Bolsa:";
    const vector<unique_ptr<Weapon>>& inv = knight.getInventory();
    if (inv.empty()) {
        cout << " (vacia)" << endl;
    } else {
        cout << endl;
        for (int i = 0; i < (int)inv.size(); i++) {
            cout << "  " << (i + 1) << ". " << inv[i]->getName()
                 << " (base: " << inv[i]->getBaseDamage() << ")" << endl;
        }
    }
}


int SimulationEngine::getHitChance(const Weapon& w) const {
    string type = w.getStrategyType();
...
string SimulationEngine::getNarrativeLine(const string& roomType) const {
    if (roomType == "aldea")    return "Los aldeanos susurran historias de tiempos mas oscuros.";
    if (roomType == "bosque")   return "El viento entre los arboles suena como lamentos olvidados.";
    if (roomType == "castillo") return "Sombras se mueven en las paredes del castillo. Algo acecha.";
    if (roomType == "cuevas")   return "El goteo del agua resuena en la oscuridad. Algo respira cerca.";
